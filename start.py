    ...
    if check_DHCP_daemon():
        if check_local_network():
            if check_internet_connection():
                ...
                subprocess.call(["mpc", "add", "error_no_internet.mp3"])
            ...
            subprocess.call(["mpc", "clear"])
            subprocess.call(["mpc", "repeat", "off"])
            subprocess.call(["mpc", "add", "error_no_network.mp3"])
            ...
            if input_type == "button":
                ...
                subprocess.call(["mpc", "clear"])
                subprocess.call(["mpc", "repeat", "off"])
                subprocess.call(["mpc", "add", "setup_wps.mp3"])
                subprocess.call(["mpc", "add", "waiting.mp3"])
                subprocess.call(["mpc", "play"])
                log_message("User selected WPS setup. Waiting for WPS activation and confirmation...")
                
                btn_sw.wait_for_press(timeout=120)
                
                if btn_sw.is_pressed:
                    log_message("WPS confirmed by user.")
                    subprocess.call(["mpc", "clear"])
                    subprocess.call(["mpc", "repeat", "off"])
                    subprocess.call(["mpc", "add", "setup_wps_confirm.mp3"])
                    subprocess.call(["mpc", "play"])
                    time.sleep(11)
                    run_auto_wps_script()
                else:
                    log_message("WPS confirmation timeout.")
                    subprocess.call(["mpc", "clear"])
                    subprocess.call(["mpc", "add", "error_no_interaction.mp3"])
                    subprocess.call(["mpc", "play"])
                    time.sleep(18)
                    subprocess.call(["sudo", "shutdown", "-h", "now"])
                ...
                subprocess.call(["mpc", "clear"])
                subprocess.call(["mpc", "repeat", "off"])
                subprocess.call(["mpc", "add", "setup_hotspot.mp3"])
                subprocess.call(["mpc", "add", "waiting.mp3"])
                subprocess.call(["mpc", "play"])
                run_auto_fallback_script()
            else:
